...
class PartialCirclesTest(unittest.TestCase):
    def test_make_2d_partial_circles(self):
        ...
        self.assertEqual(len(x), 400, "Wrong number of points generated.")
        self.assertEqual(len(y), 400, "Wrong number of point classes generated.")
        self.assertEqual(min(y), 0, "Class less than 0 found.")
        self.assertEqual(max(y), 4, "Class greater than 5 found.")
        self.assertEqual(len(circle_locs), 5, "Wrong number of circles generated.")
        ...
        for i in range(0, len(x)):
            ...
            point_class = y[i]
            self.assertLess(math.dist(x[i], circle_locs[point_class]), 12, "For class " + str(point_class) + " point " + str(x[i]) + " is " + str(math.dist(x[i], circle_locs[point_class])) + " away from center " + str(circle_locs[point_class]))
